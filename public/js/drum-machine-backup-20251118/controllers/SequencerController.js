...
export class SequencerController {
  constructor(config) {
    ...
    this.audioEngine = config.audioEngine; // AudioEngine instance
    ...
    // Load saved patterns and populate dropdown
  ...
  bindEvents() {
    ...
    this.controlsView.on('load', this.handleLoad.bind(this));
  ...
  async start() {
    try {
      ...
      if (this.audioEngine && this.audioEngine.context.state === 'suspended') {
        await this.audioEngine.context.resume();
      }
  ...
  /**
   * Load saved patterns from storage and populate dropdown
   */
  loadSavedPatterns() {
    const patterns = this.storage.listAll();
    this.controlsView.updatePatternList(patterns);
  }
   ...
   */
  handleSave() {
    const name = this.controlsView.getPatternName();
    if (!name || name.trim() === '') {
      alert('Please enter a pattern name');
      return;
    }

    const success = this.storage.save(name, this.model);
    if (success) {
      console.log(`Pattern "${name}" saved`);
      this.loadSavedPatterns();
      this.controlsView.clearPatternName();
    } else {
   ...
   * Handle load pattern
   */
  handleLoad({ patternName }) {
    if (!patternName || patternName === 'keep') {
      return;
    }

    const data = this.storage.load(patternName);
    if (data) {
      this.model.fromJSON(data);
      this.render();
      console.log(`Pattern "${patternName}" loaded`);
    } else {
      alert(`Failed to load pattern "${patternName}"`);
    }
  }
   ...
   */
  handleDelete() {
    const patternName = this.controlsView.getSelectedPattern();
    if (!patternName || patternName === 'keep') {
    ...
    if (confirm(`Delete pattern "${patternName}"?`)) {
      const success = this.storage.delete(patternName);
      if (success) {
        console.log(`Pattern "${patternName}" deleted`);
        this.loadSavedPatterns();
      } else {
